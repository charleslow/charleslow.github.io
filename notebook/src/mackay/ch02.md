...
The evaluation of the infinite sum (in line 3 above) uses the standard geometric series for $|x| < 1$, differentiates the result, and the multiplies by $x$ for the desired expression.
$$
\begin{align*}
    \sum_{n=0}^\infty &= \frac{1}{1-x}\\
    \frac{d}{dx} \left( \sum_{n=0}^\infty x^n \right) &= \frac{d}{dx} (1-x)^{-1}\\
...
For the first ball:
$$
    P(\text{Ball 1 is white}) = \frac{w}{w+b}
$$

For the second ball:
$$
\begin{align*}
    P(\text{Ball 2 is white}) &= \frac{w}{w+b} \left( \frac{w-1}{w+b-1} \right) + 
...
Observe that the coin touches the grid if the centre of the circle is $< \frac{a}{2}$ distance from the side of a grid.

Therefore, the invalid region for the centre of the circle is a rectangular border running around the square of thickness $a/2$. The valid region for the centre of the circle is thus a smaller square of length $b-a$ in the middle of the $b$ square. See diagram for illustration.
...
$$
    P(\text{Coin not touching grid}) = \frac{(b-a)^2}{b^2} = \left( 1 - \frac{a}{b} \right)^2
$$
...
- Let $\theta \sim \text{Unif}(0, \pi)$ be the angle the needle makes with the parallel lines
- Let $d \sim \text{Unif}(0, b/2)$ be the distance from the needle's centre to the nearest line
...
Notice that this formula works even if $\theta > \frac{\pi}{2}$, because $\sin \theta = \sin (\pi - \theta)$.

With a fixed $\theta$, the problem is reduced to having a needle of length $a \sin \theta$ that is always horizontal. In this case, the probability that the needle crosses a line is:
$$
    P(d \leq \frac{a}{2} \sin \theta | \theta) = \frac{a}{2} \sin \theta \div \frac{b}{2} = \frac{a}{b} \sin \theta  
$$

Thus the desired probability is:
$$
\begin{align*}
    \int_0^\pi P(d \leq \frac{a}{2} \sin \theta | \theta) P(\theta) \, d \theta
    &= \int_0^\pi \frac{a}{b} \sin \theta \frac{1}{\pi} \, d\theta\\
    &= \frac{2a}{b \pi} \ \ \ \square
\end{align*}
$$
...
In general, a triangle can be constructed if and only if the length of the longest segment is not greater than the length of the two shorter segments added up. For a unit length triangle, this means that no line segment can be $>0.5$ in length.

Let $X,Y \sim \text{Unif}(0,1)$ be independent random variables denoting the two points of line breaks. To reason about line segments, we need to choose $X < Y$. By symmetry, we can just multiply the result by $2$ at the end to cover the converse case.
...
For a valid triangle, all three line segments must be less than $0.5$. This gives a system of linear inequalities:
$$
\begin{align*}
    X &< 1/2 \\
    Y - X &< 1/2\\
...
We can visualize the valid region as a shaded region on a unit square, where the x-axis denotes $X$ and the y-axis denotes $Y$. The three constraints simplify to $X < 1/2$, $Y > 1/2$, and $Y < X + 1/2$.
...
As we can see, the area of the valid region is $1/8$. Doubling the probability for symmetry gives a final probability of $1/4$. Thus there is a 25% chance we form a valid triangle.

---

> **Exercise 2.34.**$^{[2]}$ An unbiased coin is flipped until one head is thrown. What is the expected number of tails and the expected number of heads?
> 
...
$$
    \E(T) = \frac{1}{2} \sum_{t=1}^\infty \left( \frac{1}{2} \right)^t \cdot t
$$

We can either compute this value directly, or use the recursion trick and law of total expectation to compute it. Let us define $Y=1$ as the event that the first coin flip is heads, and condition on the random variable $Y$. Then:
$$
\begin{align*}
    \E(T) &= \E(T|Y=0) \cdot P(Y=0) + \E(T|Y=1) \cdot P(Y=1)\\
    &= 0.5 \cdot (1 + \E(T)) + 0.5 \cdot (0)\\
    \E(T) &= 1\\
\end{align*}
$$

So the expected number of tails is 1. Note that for the $P(Y=0)$ case, after the first tails, the random variable $T$ reverts to the original case. Hence the expected value is just $1$ (for the tail we just rolled) plus $\E(T)$ again.
...
This problem is called the inspection paradox. There is a visualisation at [charleslow.github.io/inspection-paradox](https://charleslow.github.io/inspection-paradox) to get a feel for it.

The answer to the first part is $6$. We can see this by computing the expected value:
$$
\begin{align*}
    \E(X) = \frac{1}{6} \sum_{x=1}^\infty (\frac{5}{6})^x \cdot x
\end{align*}
$$
...
$$
\begin{align*}
    \E(X) &= \frac{1}{6} \cdot 1 + \frac{5}{6} (1 + \E(X))\\
    \E(X) &= 6\\
\end{align*}
$$
...
The probability that $P(F > B) = 0.5$, since we have no prior reason to believe one way or another.

There are two ways to get $P(F>B | F>A)$.

First, observe that there are $3! = 6$ orderings, each of equal probability:
- $A < B < F$
- $B < A < F$
- $A < F < B$
- And so on

With the information that $F>A$, we are restricted to $3$ orderings (shown above). Out of these 3, 2 of the options have $F > B$. Thus the answer is $2/3$.

The other way is to use bayes theorem.
$$
\begin{align*}
    P(F > B | F>A) &= \frac{P(F>B \cap F>A)}{P(F>A)}\\
\end{align*}
$$

The denominator is $1/2$. For the numerator, we can think of it as the probability that "Fred is the eldest". Since each person has equal probability of being the eldest, the numerator is $1/3$. So the answer is $2/3$ again.
...
Let $A=1$ denote that the first statement is true, and $B=1$ denote that the second person says "yes" (i.e. affirms the statement).
...
Now we want $P(A=1 | B=1)$. Note that $P(B=1 | A=1) = 1/3$ (truthfully confirms a true statement) and $P(B=1 | A=0) = 2/3$ (lies about a false statement). Using bayes theorem:
$$
\begin{align*}
    P(A=1 | B=1) &= \frac{P(B=1 | A=1)P(A=1)}{P(B=1)}\\
    &= \frac{1/3 \cdot 1/3}{1/3 \cdot 1/3 + 2/3 \cdot 2/3}\\
    &= 1/5
\end{align*}
$$
